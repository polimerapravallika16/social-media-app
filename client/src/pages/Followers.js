...
export default function Followers() {
    ...
    const fetchLists = async () => {
      try {
        const [followersRes, followingRes] = await Promise.all([
          fetch(`http://https://social-media-app-production-ef17.up.railway.app/api/users/followers/${myId}`),
          fetch(`http://https://social-media-app-production-ef17.up.railway.app/api/users/following/${myId}`),
        ]);
  ...
  const handleFollow = async (id) => {
    try {
      const res = await fetch(`http://https://social-media-app-production-ef17.up.railway.app/api/users/follow/${id}`, {
        method: "POST",
        headers: {
