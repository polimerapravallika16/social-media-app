...
export default function Following() {
  ...
  useEffect(() => {
    ...
    const fetchFollowing = async () => {
      try {
        const res = await fetch(`https://social-media-app-production-ef17.up.railway.app/api/users/following/${myId}`);
  ...
  const handleUnfollow = async (id) => {
    try {
      const res = await fetch(`https://social-media-app-production-ef17.up.railway.app/api/users/unfollow/${id}`, {
        method: "POST",
        headers: {
