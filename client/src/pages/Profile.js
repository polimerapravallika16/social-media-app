...
export default function Profile() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const res = await fetch(`https://://social-media-app-production-ef17.up.railway.app/api/users/${userId}`);
        const userData = await res.json();
        setData(userData);
        setIsOwnProfile(userId === currentUserId);

        const [followersRes, followingRes] = await Promise.all([
          fetch(`https://://social-media-app-production-ef17.up.railway.app/api/users/followers/${userId}`),
          fetch(`https://://social-media-app-production-ef17.up.railway.app/api/users/following/${userId}`),
        ]);
        ...
        if (currentUserId && currentUserId !== userId) {
          try {
            const myFollowingRes = await fetch(`https://://social-media-app-production-ef17.up.railway.app/api/users/following/${currentUserId}`);
            ...
            {!isOwnProfile && (
              <button
                onClick={async () => {
                  ...
                  try {
                    const res = await fetch(`https://://social-media-app-production-ef17.up.railway.app/api/users/${prev ? 'unfollow' : 'follow'}/${userId}`, {
                      method: "PATCH",
